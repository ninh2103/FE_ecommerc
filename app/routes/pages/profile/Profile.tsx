...
export default function ProfilePage() {
  ...
  const {
    ...
    setValue,
    watch
  } = useForm<UpdateUserProfileBodyType>({
  ...
  const {
    ...
    formState: { errors: errorsDisable, isSubmitting: isSubmittingDisable },
    setError: setErrorDisable
  } = useForm<Disable2FABodyType>({
    ...
    const setupQrFromProfile = async () => {
      if (profile && profile.totpSecret) {
        try {
          ...
          const uri =
            (profile as any).otpAuthUri ??
            `otpauth://totp/${encodeURIComponent(label)}?secret=${profile.totpSecret}&issuer=${encodeURIComponent(issuer)}&algorithm=SHA1&digits=6&period=30`
  ...
  const inputCls =
    'w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400'
  return (
        ...
        <aside className='rounded-xl ring-1 ring-slate-200 bg-white h-max order-2 lg:order-none lg:sticky lg:top-28'>
          ...
          <nav className='py-2'>
              ...
              {[
                ...
                <li key={item.key} className='px-4'>
                  <button
                    type='button'
                    onClick={() => {
                      setActiveTab(item.key as 'update' | 'password' | 'security' | 'disable2fa' | 'orders')
                      if (item.href) {
                        navigate(item.href)
                      }
                    }}
            ...
            <div className='px-4 py-2 border-t mt-2'>
              <button
                className='w-full text-left px-3 py-2 rounded-md hover:bg-slate-50 text-sm text-rose-600'
            ...
            <div className='p-4 grid grid-cols-1 gap-4'>
              ...
              <div>
                <Button
                  variant='outline'
                  className='text-slate-900'
                  onClick={handleSubmitPwd(onSubmitPwd)}
                  disabled={isSubmittingPwd}
                >
                  {isSubmittingPwd ? <LoadingSpinner /> : 'Change password'}
                </Button>
              </div>
            </div>
          </section>
          ...
          <section className='rounded-xl ring-1 ring-slate-200 bg-white order-1 lg:order-none'>
            ...
            <div>
              <Button
                variant='outline'
                className='text-slate-900'
                onClick={handleSubmitDisable(onSubmitDisable2FA)}
                disabled={isSubmittingDisable}
              >
                {isSubmittingDisable ? <LoadingSpinner /> : 'Disable 2FA'}
              </Button>
            </div>
          </section>
        )}
